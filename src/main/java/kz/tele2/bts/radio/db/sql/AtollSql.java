...
public class AtollSql {

    public final static String SITES_BY_NAMES = """
            with source
                     as (select case
            ...
            group by name
            """;
    public final static String AZIMUTHS = """
        with w (site, cell, azimuth, modified_date) as
                 (select site_name, tx_id, azimut, modified_date
                  from xgtransmitters
                  where active = 1
                  union all
                  select site_name, tx_id, azimut, modified_date
             ...
             b as
                 (select iif(site like 'ERBS%' or site like 'GRBS%', substring(site, 6, 5), site) as site,
                         ...
                         azimuth                                                                  as value,
                         modified_date
                  from a where sector is not null),
             c as (SELECT site,
                          sector,
                          value,
                          ROW_NUMBER() OVER (PARTITION BY site, sector ORDER BY modified_date DESC) AS rn
                   FROM b)
             select upper(site) as site, upper(sector) as sector, value from c where rn = 1
            """;

    public final static String HEIGHT = """
            with w (site, cell, height, modified_date) as
                     (select site_name, tx_id, height, modified_date
                      from xgtransmitters
                      where active = 1
                      union all
                      select site_name, tx_id, height, modified_date
    ...
    public static final String TRANSPORT_SITES = """
            ...
            """;
    public static final String TRANSPORT_SITE_NAMES = """
            select SUBSTRING(REPLACE(REPLACE(site_name, '5G_', ''), 'A-', ''), 0, 7) as name from basestation where latitude is not null and longitude is not null
            """;
}
